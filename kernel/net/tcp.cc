...
namespace net
{
    ...
    static uint32_t rx_head = 0; // next write
    static uint32_t rx_tail = 0; // next read
                             ...
                             const uint8_t *data, uint16_t data_len)
    {
        ...
        if (data && data_len > 0)
            memcpy(buf + sizeof(TcpHeader), data, data_len);
    ...
    void tcp_receive(const uint8_t *segment, uint16_t length, uint32_t src_ip)
    {
        ...
        if (ntoh16(hdr->dest_port) != conn.local_port)
            return;

        uint16_t hdr_bytes = (uint16_t)(hdr->data_offset * 4);
        if (hdr_bytes < sizeof(TcpHeader) || hdr_bytes > length)
            return;
